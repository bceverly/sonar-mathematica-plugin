...
public class PatternAndDataStructureDetector extends BaseDetector {
    ...
    private static final Pattern BLANKSEQUENCE = Pattern.compile("\\b\\w+__(?![a-zA-Z_])"); //NOSONAR - Possessive quantifiers prevent backtracking
    private static final Pattern OPTIONAL_PATTERN = Pattern.compile("\\b(\\w+)_:(\\w++)"); //NOSONAR - Possessive quantifiers prevent backtracking
    private static final Pattern PATTERN_NAME_USAGE = Pattern.compile("\\b(\\w+)_([a-zA-Z]\\w*+)?+"); //NOSONAR
    ...
    private static final Pattern REPEATED_PATTERN_NAME = Pattern.compile("\\{\\s*+(\\w+)_,\\s*+\\1_\\s*+\\}"); //NOSONAR
    // FIXED: Ultra-safe pattern with possessive quantifiers to prevent backtracking
    // Using \w++ and \s*+ ensures no backtracking on these quantifiers
    private static final Pattern ALTERNATIVES_COUNT = Pattern.compile("\\w++:\\s*+\\("); //NOSONAR - Possessive quantifiers prevent backtracking
    private static final Pattern CASES_LARGE_LIST = Pattern.compile("Cases\\s*+\\[\\s*+Range\\s*+\\[\\s*+(\\d++)\\s*+\\]"); //NOSONAR

    // Pre-compiled patterns for List/Array Rules
    private static final Pattern LIST_INDEXING = Pattern.compile("(\\w++)\\s*+\\[\\[\\s*+(-?+\\d++|\\w++)\\s*+\\]\\]"); //NOSONAR
    private static final Pattern NEGATIVE_INDEX = Pattern.compile("\\[\\[\\s*+-\\s*+(\\d++|\\w++)\\s*+\\]\\]"); //NOSONAR
    private static final Pattern PART_ASSIGNMENT = Pattern.compile("([^\\w]\\{[^}]+\\}|[A-Z]\\w*+\\s*+\\[[^\\]]+\\])\\s*+\\[\\["); //NOSONAR
    // Note: [^]]* before Join must be non-possessive to allow backtracking to find Join
    private static final Pattern JOIN_IN_LOOP = Pattern.compile("(Do|While|Table)\\s*+\\[[^\\]]*Join\\s*+\\["); //NOSONAR
    private static final Pattern FLATTEN_USAGE = Pattern.compile("Flatten\\s*+\\[\\s*+\\{\\s*+([^,}]+)(?:,\\s*+([^,}]+))*+\\s*+\\}\\s*+\\]"); //NOSONAR
    private static final Pattern LENGTH_IN_DO = Pattern.compile("Do\\s*+\\[[^,]*,\\s*+\\{[^,]*,\\s*+[^,]*,\\s*+Length\\s*+\\["); //NOSONAR
    private static final Pattern REVERSE_TWICE = Pattern.compile("Reverse\\s*+\\[\\s*+Reverse\\s*+\\["); //NOSONAR
    private static final Pattern SORT_GREATER = Pattern.compile("Sort\\s*+\\[\\s*+\\w++\\s*+,\\s*+Greater\\s*+\\]"); //NOSONAR
    private static final Pattern EXTRACT_POSITION = Pattern.compile("Extract\\s*+\\[\\s*+\\w++\\s*+,\\s*+Position\\s*+\\["); //NOSONAR
    private static final Pattern NESTED_PART = Pattern.compile("(\\w++)\\s*+\\[\\[\\s*+\\w++\\s*+\\]\\]\\s*+\\[\\["); //NOSONAR

    // Pre-compiled patterns for Association Rules
    private static final Pattern ASSOC_ACCESS = Pattern.compile("([a-zA-Z]\\w*+)\\s*+\\[\\s*+\"[^\"]+\"\\s*+\\]"); //NOSONAR
    ...
    private static final Pattern LOOKUP_MISSING = Pattern.compile("Lookup\\s*+\\[\\s*+\\w++\\s*+,\\s*+[^,]+,\\s*+Missing\\s*+\\[\\s*+\\]\\s*+\\]"); //NOSONAR
    private static final Pattern GROUPBY_USAGE = Pattern.compile("GroupBy\\s*+\\[\\s*+\\w++\\s*+,\\s*+[^,]+\\s*+\\](?!\\s*,)"); //NOSONAR
    ...
    public void detectAlternativesTooComplex(SensorContext context, InputFile inputFile, String content) {
        try {
            ...
            while (matcher.find()) {
                processAlternativesMatch(context, inputFile, content, matcher);
            }
        } catch (Exception e) {
    ...
    private void processAlternativesMatch(SensorContext context, InputFile inputFile, String content, Matcher matcher) {
        ...
        if (endPos > startPos) {
            ...
            int pipeCount = countPipes(patternContent);

            // Report if 10 or more alternatives (9+ pipes means 10+ alternatives)
            if (pipeCount >= 9) {
                reportComplexAlternatives(context, inputFile, content, matcher.start(), pipeCount);
            }
        }
    }
    ...
    private int findMatchingClosingParen(String content, int startPos) {
        ...
        return (parenDepth == 0) ? endPos : startPos;
    }

    private int countPipes(String text) {
        int count = 0;
    ...
    private void reportComplexAlternatives(SensorContext context, InputFile inputFile,
                                          String content, int position, int pipeCount) {
        int line = calculateLineNumber(content, position);
